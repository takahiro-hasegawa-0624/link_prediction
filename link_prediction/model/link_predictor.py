...
class Link_Prediction_Model():
    ...
    def model_evaluate(self, validation=False, save=True):
        ...
        if validation is True:
            ...
            neg_edge_index = self.data['val_neg_edge_index']
            edge_index = torch.cat([pos_edge_index, neg_edge_index], dim = -1)
            val_link_probs = self.best_decode_model.decode(z, decode_edge_index=edge_index).cpu().detach().clone()
            val_link_labels = my_utils.get_link_labels(pos_edge_index, neg_edge_index).cpu()

            pos_edge_index = self.data['test_pos_edge_index']
            neg_edge_index = self.data['test_neg_edge_index']
            edge_index = torch.cat([pos_edge_index, neg_edge_index], dim = -1)
            test_link_probs = self.best_decode_model.decode(z, decode_edge_index=edge_index).cpu().detach().clone()
            ...
            neg_edge_index = self.data['val_neg_edge_index']
            edge_index = torch.cat([pos_edge_index, neg_edge_index], dim = -1)
            val_link_probs = self.decode_model.decode(z, decode_edge_index=edge_index).cpu().detach().clone()
            val_link_labels = my_utils.get_link_labels(pos_edge_index, neg_edge_index).cpu()

            pos_edge_index = self.data['test_pos_edge_index']
            neg_edge_index = self.data['test_neg_edge_index']
            edge_index = torch.cat([pos_edge_index, neg_edge_index], dim = -1)
            test_link_probs = self.decode_model.decode(z, decode_edge_index=edge_index).cpu().detach().clone()
            test_link_labels = my_utils.get_link_labels(pos_edge_index, neg_edge_index).cpu()
