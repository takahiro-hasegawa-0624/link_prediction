...
class Link_Prediction_LINE():
    ...
    def __init__(self, dataset_name='', data=None, val_ratio=0.05, test_ratio=0.1, data_dir='../data'):
        ...
        self.all_pos_edge_index, self.train_pos_edge_adj_t, self.y_true, self.y_train_cpu, self.mask = my_utils.data_processor(data=self.data, undirected=True, val_ratio=val_ratio, test_ratio=test_ratio)
    ...
    def train(self):
        ...
        loss = -torch.log(torch.cat([torch.sigmoid(z[:self.data.train_pos_edge_index.size(1)]), torch.sigmoid(-z[self.data.train_pos_edge_index.size(1):])], dim=-1)).mean()
    ...
    def model_evaluate(self, validation=False, save=True, fig_show=True, fig_size=4):
        ...
        if validation is True:
            ...
            neg_edge_index = self.data['val_neg_edge_index']
            edge_index = torch.cat([pos_edge_index, neg_edge_index], dim = -1)
            z = self.best_decode_model(edge_index)
            val_link_probs = torch.sigmoid(z).cpu().detach().clone()
            ...
            edge_index = torch.cat([pos_edge_index, neg_edge_index], dim = -1)
            z = self.best_decode_model(edge_index)
            test_link_probs = torch.sigmoid(z).cpu().detach().clone()
