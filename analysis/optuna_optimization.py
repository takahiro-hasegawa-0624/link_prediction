...
def get_scheduler(trial, model, args):    
    ...
    if args.decode_model != 'Cat_Linear_Decoder':
        ...
        scheduler['decoder_bias'] = torch.optim.lr_scheduler.ExponentialLR(model.optimizer['decoder_bias'], gamma=0.992)
...
def main():
    ...
    parser.add_argument('--weight_decay_max', type=float, default=1e-6)
    ...
    if args.decode_model != 'Cat_Linear_Decoder':
        ...
        scheduler['decoder_bias'] = torch.optim.lr_scheduler.ExponentialLR(model.optimizer['decoder_bias'], gamma=0.92)
    if args.decode_model == 'Cat_Linear_Decoder':
    ...
    model.run_training(num_epochs=1000, print_log=True, save_dir=current_dir)
